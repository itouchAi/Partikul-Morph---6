      ...
      */}
      {(showInfoPanel && audioMode !== 'none') && !isDrawing && (
          <div 
            ...
            className={`
                cursor-pointer preserve-3d transition-all duration-700 ease-[cubic-bezier(0.4,0,0.2,1)] transform-gpu z-[200]
                ${isWidgetMinimized ? '-translate-y-[100px]' : ''} ${hideLeftClass}
                ${isInfoExpanded ? 'rotate-y-180' : 'rotate-y-0'}
            `}
            style={{
                position: 'fixed',
                // Using transform translation for centering allows smoother pixel-to-percentage transition
                // Collapsed: Top 230px, Left 80px (approx left-20), No translate
                // Expanded: Top 50%, Left 50%, Translate -50% -50%
                top: isInfoExpanded ? '50%' : '230px',
                left: isInfoExpanded ? '50%' : '80px',
                width: isInfoExpanded ? '500px' : '230px', // Reduced from 256px (-10%)
                height: isInfoExpanded ? '500px' : '252px', // Reduced from 280px (-10%)
                transform: isInfoExpanded 
                    ? 'translate(-50%, -50%) rotateY(180deg)' 
                    : `translate(0, 0) rotateY(0deg) ${isWidgetMinimized ? 'translateY(-100px)' : ''}`,
                overflow: 'visible' // CRITICAL: Allows disk to stick out
            }}
          >
              {/* --- FRONT FACE (Small Widget) --- */}
              <div className={`
                    absolute inset-0 backface-hidden rounded-3xl border backdrop-blur-xl shadow-2xl flex flex-col 
              ...
              `}
              style={{ overflow: 'visible' }} // CRITICAL: Allows disk to stick out
              >
                      ...
                      {songInfo && !isLoadingInfo && ( 
                          <div className="w-full px-1 flex flex-col gap-2 mt-1">
                              ...
                              <div className={`text-xs leading-snug text-white/90 font-medium italic line-clamp-4 text-center drop-shadow-md`}>
                                  <span className="font-bold not-italic opacity-80 text-[10px] mr-1 block mb-0.5 text-blue-300">Song Analysis (En):</span>
                                  "{songInfo.meaningEN}"
                              </div>
                          </div>
                      )}
              ...
              <div 
                className={`absolute inset-0 backface-hidden rounded-3xl border backdrop-blur-3xl shadow-2xl flex flex-col ${isLightMode ? 'bg-white/85 border-black/10 text-black' : 'bg-[#111]/85 border-white/10 text-white'}`} 
                  ...
                  <div className="flex-1 p-8 pt-6 overflow-y-auto custom-thin-scrollbar relative z-10">
                      {songInfo ? ( 
                          <div className="space-y-8"> 
                              ...
                              <div> 
                                  <h4 className="font-bold text-lg mb-2 opacity-80 border-b border-current pb-1 inline-block uppercase tracking-wider">Song Analysis (En)</h4> 
